...
export default function NavLink({children, link, href}: {children: ReactNode, link: string, href: string}){
  ...
  return(
    <Link className={`flex flex-col gap-1 justify-center items-center py-1 rounded-sm max-w-[53.33%] m-auto ${resolvedParam === link ? "bg-blue-50 text-blue-500" : "text-neutral-950"}`} href={href}>
      {children}
    </Link>
  );
}